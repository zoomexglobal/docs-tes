...
1. REST API
   - Add [Get Bonus Usage Records](../v3/account/bonus-usage) API.

## 2026-4-7
### Affiliate

1. REST API
   - Add [Get direct customers trade volume](../v3/broker/broker-trade-volume) and [Get direct customers asset balance](../v3/broker/broker-asset-balance).

## 2026-3-30
### Market

1. REST API
   - The API for [Get Instruments Info](../v3/market/instrument) has added an `offlineTime` field, displaying the offline time of the symbol;
   - The API for [Get Tickers](../v3/market/tickers) has added the `fundingIntervalHour` and `fundingCap` fields;

### Account

1. REST API
   - Add [Get Deposit Address](../v3/account/deposit-address) API.
   - Support querying by `coin`; pass `chain` to return one chain address, omit `chain` to return all supported chain addresses.
