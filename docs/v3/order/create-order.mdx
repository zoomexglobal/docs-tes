...
|orderFilter |false |string |If it is not passed, Order by default.<ul><li>Order</li><li>tpslOrder: Spot TP/SL order, the assets are occupied even before the order is triggered</li><li>StopOrder: Spot conditional order, the assets will not be occupied until the price of the underlying asset reaches the trigger price, and the required assets will be occupied after the Conditional order is triggered</li>Valid for spot only</ul>|
