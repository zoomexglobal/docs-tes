...
|qty |**true** |string |Order quantity. <ul><li>Trading account<ul><li>Spot: set `marketUnit` for market order qty unit, `quoteCoin` for market buy by default, `baseCoin` for market sell by default</li><li>Futures: always use base coin as unit</li></ul></li><li>Classic account<ul><li>Spot: the unit of qty is quote coin for market buy order, for others, it is base coin</li><li>Futures: always use base coin as unit</li></ul></li></ul>|
|marketUnit |false |string |The unit for `qty` when create **Spot market** orders for **Trading account**. Currently, TP/SL and conditional orders are not supported.<li>`baseCoin`: for example, buy BTCUSDT, then "qty" unit is BTC</li><li>`quoteCoin`: for example, sell BTCUSDT, then "qty" unit is USDT</li>|
