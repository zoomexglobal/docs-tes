...
|:----- |:-------|:-----|------ |
|[accountType](../enum#accounttype) |**true** |string |Account type <ul><li>uta: `UNIFIED`</li><li>Classic account: `CONTRACT`</li></ul> |
...
|> accountIMRate |string |Account IM rate <li>All account wide fields are <b>not</b> applicable to <br/>UTA(isolated margin), <br/>classic account(CONTRACT)</li>|
|> accountMMRate |string |Account MM rate||> totalEquity |string |Equity of account converted to usd：Account Margin Balance Base Coin + Account Option Value Base Coin.|
...
|>> availableToWithdraw |string |**Note:** this field is deprecated for `accountType=UNIFIED` from 9 Jan, 2025 <ul><li>Derivatives available balance: <br/><b>isolated margin</b>: walletBalance - totalPositionIM - totalOrderIM - locked - bonus<br/><b>cross & portfolio margin</b>: look at field `totalAvailableBalance`(USD), which needs to be converted into the available balance of accordingly coin through index price</li></ul>|
