...
- **注意**: 當`category`等於`linear`或`inverse`, 您必需指定两個參數的其中一個。
:::
...
|settleCoin |false|string |結算幣種 <ul><li>對於`linear` & `inverse`: 若不傳`symbol`和`baseCoin`，該字段**必傳**。 </li></ul> |
...
<TabItem value="future" label="Linear/Inverse" default>
    <table border="0.8">
        ...
        <tr>
            <td>> orderLinkId</td><td>string</td><td>用户自自定义订单ID</td>
        </tr>
    </table>
</TabItem>
</Tabs>
