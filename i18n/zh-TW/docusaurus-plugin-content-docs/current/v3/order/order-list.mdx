...
:::info
* **7天內的訂單**: <br/>交易帳戶(除了inverse)支持查詢除了"Cancelled"(完全取消), "Rejected", "Deactivated"以外的[終態訂單](../enum#orderstatus) <br/>统一账户1.0(inverse)和經典帳戶支持查詢活動態+終態的訂單
* **24小時的订单**: <br/>交易帳戶(除了inverse): 對於完全取消(Cancelled),以及"Rejected", "Deactivated"的訂單僅支持查詢過去24小時的訂單記錄
* **7天外的訂單**: 所有账户都只能查詢到有過成交的訂單, 即完全成交, 部分成交但最終取消的訂單
* 交易帳戶(除了inverse) 支持查詢過去2年的數據
:::
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型 <ul><li>交易帳戶：`spot`, `linear`, `inverse`</li><li>普通帳戶: `linear`, `inverse`, `spot`</li></ul> |
...
|[orderStatus](../enum#orderstatus) |false |string |訂單狀態 <ul><li>經典帳戶`spot`: 該字段無效</li><li>交易帳戶(除了inverse): 不傳則默認查詢所有終態訂單</li><li>交易帳戶(inverse)和經典帳戶: 不傳則默認查詢活動態+終態的訂單</li></ul> |
