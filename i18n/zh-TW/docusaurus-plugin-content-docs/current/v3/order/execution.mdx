...
:::tip
* 單筆訂單可能會有多次成交.
* 您可以通過指定symbol, baseCoin, orderId 字段來查詢。如果您使用多字段組合，系統的查詢優先級如下: orderId > symbol.
:::

:::info
* 交易帳戶支持查詢過去730天的歷史成交紀錄
:::
...
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |**true** |string |產品類型 <ul><li>交易帳戶：`spot`, `linear`, `inverse` </li><li>經典帳戶: `spot`, `linear`, `inverse`</li></ul> |
