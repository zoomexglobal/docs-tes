...
:::tip
- 交易帳戶: 支持查詢已成交, 取消和拒絕類型的最近500條訂單信息對於spot, linear, inverse類別.
...
* 針對交易帳戶, 服務器重啓或發佈後請使用[查詢歷史訂單](order-list#)接口去查詢已成交，取消和拒絕類型的訂單信息
:::
...
|openOnly |false |integer |<ul><li>`0`(默認): 交易帳戶, 經典帳戶: 僅查詢活動委託訂單, 比如New, PartiallyFilled訂單</li><li>`1`: 交易帳戶</li><li>`2`: 經典帳戶:</li></ul>返回僅終態（已取消/拒絕/完全成交）的訂單數據, 每個帳戶每個category下最多保留500條紀錄. <i>如果因Bybit服務更新重啟則該部分數據會情況並重新累計, 但是訂單紀錄仍然可以從[歷史訂單](order-list#)中查詢到</i><ul><li>當查詢是按照_orderId_ 或者 _orderLinkId_時, `openOnly`入参將會被忽略</li><li>經典帳戶現貨: 該字段無效, 僅返回活動委託單</li></ul>|
...
|> AvgPrice |string |訂單平均成交價格 <li>交易帳戶: 不存在avg price場景的訂單返回`""`</li><li>經典帳戶: 不存在avg price場景的訂單返回`"0"`, 以及部分成交但最終被手動取消的訂單</li> |
...
|> marketUnit |string |交易帳戶現貨交易時給入參`qty`選擇的單位. `baseCoin`, `quoteCoin` |
