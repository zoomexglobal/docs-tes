...
|data |array |Object |
|> category |string |產品類型 <ul><li>普通帳戶: `linear`, `inverse`.</li></ul>|
|> orderId  |string |訂單ID |
...
|> avgPrice |string |訂單平均成交價格. 若沒有成交，則返回`""`. <ul><li>如果是一筆部分成交訂單且終態是Cancelled, 則avgPrice是`"0"`</li></ul> |
...
|> closeOnTrigger |boolean |觸發後平倉委託. <a href="https://www.zoomex.com/zh-TW/help-center/bybitHC_Article?language=zh_TW&id=000001050">什麼是觸發後平倉委託?</a> |
